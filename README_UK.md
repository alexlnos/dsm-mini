...
> Працює: бот, Mini App і сповіщення. Перевірено на DSM 7.2.2 з Download Station
> 4.1.2 та File Station 1.4.4. Підтримку DSM 6 написано за документацією, але на
> живому DSM 6 не перевіряли.
...
5. У відповідь прийде рядок на кшталт
   `8929377165:AAFHfZfqKDUEFdv-Yq4TJ9etz4Pp-yBU3Vg`. Це **токен**. Скопіюйте
...
Усе, що запитував майстер, лежить в одному файлі
`/var/packages/dsm-mini/var/config.env`. Найпростіше змінити значення,
встановивши пакет поверх самого себе — майстер запитає заново. Правити файл
напряму можна через SSH; після правки зупиніть і запустіть пакет у Центрі
пакетів.
