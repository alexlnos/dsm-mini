# Как поставить торрент на Synology с телефона за два тапа: Telegram-бот с Mini App вместо DSM

Вечер, метро, полчаса до дома. Коллега скинул название фильма, и хочется, чтобы к моменту, когда я дойду до дивана, он уже лежал на NAS Synology. Задача ровно в одно действие: взять magnet-ссылку и отдать её Download Station.

Спойлер: с телефона это три минуты возни. В итоге я собрал `dsm-mini` — Telegram-бот с Mini App, который ставит торренты в Download Station и показывает состояние NAS. Открытый, MIT, ставится пакетом в Центре пакетов DSM. Ниже — обзор со скриншотами, грабли недокументированного Synology API и честный разбор того, что сломалось, пока я писал это в паре с ИИ-агентом.

## Почему торренты на Synology с телефона — это боль
...
## Что получилось: Telegram-бот для Download Station
...
![Telegram Mini App для Synology: обзор интерфейса](https://raw.githubusercontent.com/alexlnos/dsm-mini/main/docs/article/walkthrough.gif)

А если открыть приложение, там оказывается заметно больше.

## Чем это отличается от DS get и веб-интерфейса DSM

| | DS get | DSM в браузере | dsm-mini |
|---|---|---|---|
| Добавить торрент с телефона | да | неудобно | два тапа |
| Выбрать файлы внутри раздачи | нет | да | да |
| Приоритеты файлов | нет | да | да |
| Состояние дисков и температура | нет | да | да |
| Уведомление о завершении | нет | нет | в чат |
| Нужен VPN или открытый DSM | да | да | нет, только HTTPS-адрес |
...
![Состояние Synology NAS в Telegram Mini App](https://raw.githubusercontent.com/alexlnos/dsm-mini/main/docs/article/01-home.png)
...
![Список торрентов Download Station на телефоне](https://raw.githubusercontent.com/alexlnos/dsm-mini/main/docs/article/02-downloads.png)
...
![Карточка торрент-задачи Synology](https://raw.githubusercontent.com/alexlnos/dsm-mini/main/docs/article/03-task.png)
...
![Выбор файлов и приоритетов внутри торрента](https://raw.githubusercontent.com/alexlnos/dsm-mini/main/docs/article/04-task-files.png)

**Добавление.** Поле для ссылки, кнопка «Из буфера», список папок. Ссылку можно и не открывая приложение — просто переслать боту.

![Добавление magnet-ссылки в Download Station](https://raw.githubusercontent.com/alexlnos/dsm-mini/main/docs/article/05-add.png)
...
![Настройка папок назначения на Synology](https://raw.githubusercontent.com/alexlnos/dsm-mini/main/docs/article/06-folders.png)

**Файлы.** File Station без веб-интерфейса: обзор папок, загрузка с телефона, переименование, копирование, перенос, удаление.

![File Station с телефона через Telegram](https://raw.githubusercontent.com/alexlnos/dsm-mini/main/docs/article/07-files.png)

![Содержимое папки на NAS Synology](https://raw.githubusercontent.com/alexlnos/dsm-mini/main/docs/article/08-files-inside.png)
...
![Диски, пулы и тома Synology: температура и здоровье](https://raw.githubusercontent.com/alexlnos/dsm-mini/main/docs/article/09-storage.png)

**Виртуальные машины и контейнеры.** Запустить и остановить. Не полноценный VMM, но когда нужно поднять машину удалённо — хватает.

![Управление виртуальными машинами Synology с телефона](https://raw.githubusercontent.com/alexlnos/dsm-mini/main/docs/article/10-vms.png)

![Контейнеры Container Manager на Synology](https://raw.githubusercontent.com/alexlnos/dsm-mini/main/docs/article/11-containers.png)

**Журнал событий DSM.** С фильтром по проблемным записям — обычно именно они и нужны.

![Журнал событий DSM в мобильном приложении](https://raw.githubusercontent.com/alexlnos/dsm-mini/main/docs/article/12-log.png)

Интерфейс и бот говорят на десяти языках, выбирается по языку клиента Telegram.

## Как это устроено: Go, React и пакет для DSM 7
...
Снаружи торчит только обратный прокси DSM на 443, а сама служба слушает `127.0.0.1`. Доступ к API проверяется дважды: подпись `initData` ключом от токена бота и числовой идентификатор в белом списке. Пустой список закрывает доступ всем, а не открывает — это ровно та ошибка, которую хочется сделать в спешке.

Для DSM служба заводит отдельного пользователя, не администратора, и только с правами на Download Station и File Station.

## Грабли Synology Web API
...
## Вайбкодинг: что реально ломается
...
Ставится пакетом через «Источники пакетов» Центра пакетов DSM, адрес по архитектуре есть в README. Нужен DSM 7 или новее, Download Station, бот от @BotFather и публичный HTTPS-адрес для Mini App — мастер установки спрашивает и объясняет каждое поле.
